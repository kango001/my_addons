...
def mainlist(item):
    ...
    itemlist.append(item.clone(title="Whats On", action="entradas", url="http://www.sport365.live/es/events/-/1/-/-/60", fanart="http://i.imgur.com/bCn8lHB.jpg?1"))
    itemlist.append(item.clone(title="Broadcasts", action="entradas", url="http://www.sport365.live/es/events/-/2/-/-/60", fanart="http://i.imgur.com/bCn8lHB.jpg?1"))
    itemlist.append(item.clone(title="Events by language ", action="idiomas", url="http://www.sport365.live/es/sidebar", fanart="http://i.imgur.com/bCn8lHB.jpg?1"))
...
def entradas(item):
    ...
    if "Redifusiones" in item.title and not fechas:
        itemlist.append(item.clone(action="", title="No event currently scheduled"))
        return itemlist
...
def play(item):
    ...
    if not url:
        platformtools.dialog_notification("Stream no disponible", "It is not possible to connect to the broadcast")
        return []
    elif "/matras.jpg" in data:
        platformtools.dialog_notification("Stream caído", "Please try again in a few minutes")
        return []
