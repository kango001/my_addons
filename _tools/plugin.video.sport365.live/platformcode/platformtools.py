...
def get_dialogo_opciones(item, default_action, strm):
    ...
    # Si no puedes ver el vídeo te informa
        ...
        if item.server != "":
            if "<br/>" in motivo:
                dialog_ok("You can not see that video because ...", motivo.split("<br/>")[0], motivo.split("<br/>")[1],
                          item.url)
            else:
                dialog_ok("You can not see that video because ...", motivo, item.url)
        else:
            dialog_ok("You can not see that video because ...", "The server where it is hosted is not",
                      "Not supported yet", item.url)
